...
func Init() {
	...
	if CompanyConfig().BaseCfg.Enable {
		globalCfg.CompanyCfg.BaseCfg.UnixSockFile = beego.AppConfig.DefaultString("company_exporter.unix_sock",
			"/dev/shm/company_exporter.sock")
		globalCfg.CompanyCfg.BaseCfg.MetricsPath = beego.AppConfig.DefaultString("company_exporter.metrics_path",
			"/metrics")
		globalCfg.CompanyCfg.BaseCfg.MetricsRouter = beego.AppConfig.DefaultString("company_exporter.metrics_router",
			"/company")
		globalCfg.CompanyCfg.BaseCfg.Timeout = time.Duration(beego.AppConfig.DefaultInt("company_exporter.timeout",
			5)) * time.Second
		filters := strings.Split(beego.AppConfig.DefaultString("company_exporter.filters", ""),
			",")
		if len(filters) > 0 {
			...
			globalCfg.CompanyCfg.BaseCfg.Filters = m
		}
	}
	...
	if CompanyHbConfig().BaseCfg.Enable {
		globalCfg.CompanyHbCfg.BaseCfg.UnixSockFile = beego.AppConfig.DefaultString("company_hb.unix_sock",
			"/dev/shm/companyhb.sock")
		globalCfg.CompanyHbCfg.BaseCfg.MetricsPath = beego.AppConfig.DefaultString("company_hb.metrics_path",
			"/metrics")
		globalCfg.CompanyHbCfg.BaseCfg.MetricsRouter = beego.AppConfig.DefaultString("company_hb.metrics_router",
			"/companyhb")
		globalCfg.CompanyHbCfg.BaseCfg.Timeout = time.Duration(beego.AppConfig.DefaultInt("company_hb.timeout",
			5)) * time.Second
		filters := strings.Split(beego.AppConfig.DefaultString("company_hb.filters", ""),
			",")
		if len(filters) > 0 {
			var m map[string]string
			m = make(map[string]string)
			...
			globalCfg.CompanyHbCfg.BaseCfg.Filters = m
		}
	}
